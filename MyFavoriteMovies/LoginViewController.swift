    ...
    private func getRequestToken() {
        ...
        let task = appDelegate.sharedSession.dataTask(with: request) { (data, response, error) in
            ...
            guard (error == nil) else {
                self.debugTextLabel.text = "Oops! There was an Error!"
                return
            }
            
            guard let data = data else{
                self.debugTextLabel.text = "Oops! No Data was Returned!"
                return
            }
            ...
            do {
             parsedResult = try! JSONSerialization.jsonObject(with: data, options: .allowFragments) as! [String : AnyObject]
                ...
                self.debugTextLabel.text = "Oops! Couldn't Parse JSON Data!"
                return
            }
            guard let requestToken = parsedResult[Constants.TMDBParameterKeys.RequestToken] else {
                self.debugTextLabel.text = "Oops! Couldn't Find Request Token!"
                return
            }
        ...
        let task = appDelegate.sharedSession.dataTask(with: <#T##URL#>, completionHandler: <#T##(Data?, URLResponse?, Error?) -> Void#>)
            ...
            func displayError(_ error: String, debugLabelText: String? = nil) {
                ...
                do {
                    ...
                    displayError("Couldn't Parse JSON Data")
                    return
                }
                guard let successKey = parsedResults[Constants.TMDBResponseKeys.Success] as? Bool, successKey == true else{
                    displayError("Key returned False")
                    return
                }
